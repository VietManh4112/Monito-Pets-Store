...
export default function Pets() {
    ...
    const showBanner = () => {
        if (flagRef.current) {
            ...
            if (flagRect < 300) {
                if (divRefs.current) {
                    divRefs.current[0]?.classList.remove('opacity-0');
        ...
        <>
            <div className="mx-auto max-w-7xl px-4 sm:px-0">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-7 py-7">
                    ...
                    <div className="block sm:hidden col-span-2"><Btn type="view" load={false}>View More</Btn></div>
                </div>

                <div ref={flagRef} className="relative overflow-hidden grid sm:grid-cols-2 bg-[var(--blue)] sm:gap-4 rounded-2xl mt-12 z-10">
                    <div ref={(el) => { divRefs.current[0] = el; }} className="sm:px-5 sm:pt-7 flex justify-center order-2 sm:order-1 opacity-0" style={{ '--fade-in': `${2}s` } as React.CSSProperties}>
                        <Image src='/bg.png' alt="banner" width={160} height={160} className="object-cover w-auto h-[342px]"></Image>
                    </div>
                    <div ref={(el) => { divRefs.current[1] = el; }} className="text-center sm:text-right mx-auto my-auto order-1 sm:order-2 opacity-0" style={{ '--fade-in': `${2}s` } as React.CSSProperties}>
                        <div className="sm:max-w-[403px] px-5 py-8 sm:px-0 sm:py-0">
                    ...
                    <div className="absolute w-[780px] h-[635px] rounded-[99px] rotate-[12deg] sm:rotate-[25deg] bg-[var(--linear)] translate-x-[-320px] sm:translate-x-[650px] translate-y-[-355px] sm:translate-y-[-250px] -z-10"></div>
                    <div className="absolute w-[790px] h-[790px] rounded-[99px] rotate-[28deg] bg-[var(--dblue)] translate-x-[-200px] sm:translate-x-[-170px] translate-y-[500px] sm:translate-y-[140px] -z-10"></div>
                </div>
            </div>
        </>
    );
